...
void testAdd1()
{
	printf("=========testAdd1(),因编译器的不同而不同\n");
	int i=3,j=3;
	// 先使用，再自增
	printf("i=%d,i++=%d\n",i,i++); // i=3,i++=3  （GCC：i=4,i++=3） 
	printf("j++=%d,j=%d\n",j++,j); // j++=3,j=3  （GCC：j=3,j++=3,相同） 
	
	// 先自增，再使用
	i=3; j=3;
	printf("i=%d,++i=%d\n",i,++i); // i=4,++i=4 （GCC：相同） 
	printf("++j=%d,j=%d\n",++j,j); // ++j=4,j=4  (GCC: ++j=4,j=3)

	// 先自增两次，再使用
	i=3; j=3;
	printf("i=%d,++i=%d,++i=%d\n",i,++i,++i); // i=5,++i=5,++i=5 (GCC:5,5,4) 
	printf("++j=%d,++j=%d,j=%d\n",++j,++j,j); // ++j=5,++j=5,j=5  (GCC:5,4,3)
	
	// P42,结论，一条语句中，相同变量，仅有一次自增或自减操作，各编译器解释一样，如下例；否则如上例。 
    int x;
    x=10;
    printf("x=%d\n",++x); // GCC：x=11，与书上的解释相同
    x=10;
...
// &&的有0为0和||的有1为1比运算符优先级更优先。例如本例的（i2--）虽然优先级更高，但是也没有该规则高。
...
void expression()
{
     ...
     if (a>b+c && b+c <=j) {
       printf("a>b+c && b+c<=j 条件为真\n");  
     }
     else {
       printf("a>b+c && b+c<=j 条件为假\n");  // ok
     }
     
     // 1<x<10与x>1 && x<10不等效，但是后者的数学意义更明确，应使用后者表示前者的数学意义。 
     int x=20;
     if (1<=x<=10) printf("1<=x<=10 条件真\n"); // ok 【1<x<10 -> 1<20<10 -> 自左向右,1<20为真=1,1<10因此最后=1】 
     else printf("1<=x<=10 条件假\n");  
     if (x>=1 && x<=10) printf("x>=1 && x<=10 条件真\n"); 
     else printf("x>=1 && x<=10 条件假\n"); // ok 
