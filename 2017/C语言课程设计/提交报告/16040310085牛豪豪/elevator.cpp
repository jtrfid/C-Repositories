...
void StateIdle(int *state)             //空置状态函数
{
	...
	if (GetCallLight(GetNearestFloor(), 0))           //门外灯亮起
	{
		SetDoor(GetNearestFloor(), 1);                 //开门
		SetCallLight(GetNearestFloor(), 0, 0);     //门外灯熄灭
		*state = DoorOpen;
	}
	if (GetCloseDoorLight())
	{
		SetCloseDoorLight(0);                         //防止死循环，消费该功能按键
		return;
	}
}
