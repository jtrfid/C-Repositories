...
void ch4_6()
{
    ...
    while(scanf("%d%d%d",&a,&b,&c) != EOF) {
      // 解法一：选择法排序：升序，每趟排序，选出本次最大者， 
      // max=a[0]，然后max与a[1]，a[2]比较,max是二者的大者。第一趟排序选出了最大者，放在a[0] 
      // 【与本题无关】， max=a[1]; 然后与a[2]比较，选出大者给a[1];三个数得到由大到小的排序。 
      max = a; 
      if (b > max) max = b;
      if (c > max) max = c;  
      printf("解法一：%d,%d,%d,最大的是: %d\n",a,b,c,max);
      
      // 解法二：冒泡法排序：降序，相邻元素比较，大者交换到后面（下面） ：
...
void ch4_9_2(float x)
{
    ...
    switch((int)(x/10.0)) 
    //switch(x/10) // 出错，switch(表达式)要求整型或字符型 
    {
      case 10: 
